...
SRC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    HERE, "..", "assets/me", "maulik-prepped.png")
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(
    HERE, "..", "assets/me", "maulik-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">Maulik557-png@github: ~$ ./Maulik-Shah.sh</text>')
...
# 1. Added font-family="monospace" to ensure predictable spacing
# 2. Moved the animated cursor inside the <text> node using relative alignment (dy)
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13" font-family="monospace">'
             f'Maulik557-png@github:~$ whoami <tspan fill="{INK}">Maulik Shah</tspan>'
             f'<tspan fill="{INK}" dy="2">█<animate attributeName="opacity" values="1;1;0;0" '
             f'keyTimes="0;0.5;0.51;1" dur="1s" repeatCount="indefinite"/></tspan></text>')
